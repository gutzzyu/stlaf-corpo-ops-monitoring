...
const AdminDashboard: React.FC<AdminDashboardProps> = ({ onBack }) => {
  ...
  const handleUpdateUserRole = async (uid: string, newRole: 'user' | 'admin') => {
    try {
  ...
  const handleDeleteUser = async (uid: string) => {
    if (!window.confirm("Are you absolutely sure? This will remove the user's profile from the registry.")) return;
    try {
  ...
  if (selectedEntry) {
    return <SummaryView entry={selectedEntry} onBack={() => setSelectedEntry(null)} />;
  }
    ...
    <div className="max-w-7xl mx-auto space-y-8">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 border-b border-slate-100 pb-8">
        <div className="space-y-4">
          <Button variant="ghost" size="sm" onClick={onBack} className="gap-2 text-slate-400 hover:text-navy-900 -ml-2 transition-all">
          ...
          <div className="space-y-2">
            <div className="flex items-center gap-2">
               ...
               <span className="text-[10px] font-black uppercase tracking-[0.4em] text-slate-300">Admin Control Unit</span>
            </div>
      ...
      {adminTab === 'personnel' && (
        <div className="grid gap-6 md:grid-cols-4">
          {[
            { label: 'Total Enrolled', value: allUsers.length, color: 'text-navy-900' },
            { label: 'Administrators', value: allUsers.filter(u => u.role === 'admin').length, color: 'text-red-500' },
            { label: 'Standard Users', value: allUsers.filter(u => u.role === 'user').length, color: 'text-blue-500' },
            { label: 'Departments', value: new Set(allUsers.map(u => u.department)).size, color: 'text-amber-600' },
          ].map((stat, i) => (
            <Card key={i} className="border-none bg-white shadow-sm overflow-hidden group">
              <div className="h-1 bg-navy-900/10 group-hover:bg-navy-900 transition-colors" />
              <CardHeader className="p-6">
      ...
      <div className="flex flex-col md:flex-row gap-4">
        <div className="relative flex-1">
          ...
          <Input 
            placeholder={adminTab === 'missions' ? "Search missions..." : "Search personnel by name, email, dept..."}
            value={searchTerm}
            ...
            <DropdownMenu>
              ...
              <DropdownMenuContent className="rounded-2xl min-w-[200px] p-2">
                 {['All', 'Draft', 'Ongoing', 'For Liquidation', 'Submitted', 'Approved', 'Rejected'].map(s => (
                   <DropdownMenuItem key={s} onClick={() => setStatusFilter(s as any)} className="rounded-xl cursor-pointer">
                     {s}
                   </DropdownMenuItem>
                 ))}
              </DropdownMenuContent>
      ...
      <AnimatePresence mode="wait">
        {adminTab === 'missions' ? (
          viewMode === 'table' ? (
              ...
              <Table>
                <TableHeader className="bg-slate-50/50">
                  <TableRow className="hover:bg-transparent border-slate-50">
                    ...
                    <TableHead className="font-black text-[9px] uppercase tracking-[0.2em] h-14 text-slate-400">Budget (PHP)</TableHead>
                    <TableHead className="font-black text-[9px] uppercase tracking-[0.2em] h-14 text-slate-400">Audit Status</TableHead>
                    <TableHead className="font-black text-[9px] uppercase tracking-[0.2em] h-14 text-slate-400 text-right pr-8">Actions</TableHead>
                  </TableRow>
                </TableHeader>
                <TableBody>
                  {loading ? (
                    <TableRow><TableCell colSpan={5} className="text-center py-20 text-xs font-black uppercase text-slate-300 tracking-widest italic animate-pulse">Syncing Satellite Data...</TableCell></TableRow>
                  ) : filteredData.length === 0 ? (
                    <TableRow><TableCell colSpan={5} className="text-center py-20 text-xs font-black uppercase text-slate-300 tracking-widest italic">No matching records found.</TableCell></TableRow>
                  ) : (
                    filteredData.map((item) => (
                      <TableRow key={item.id} className="hover:bg-slate-50/80 transition-colors border-slate-50 group">
                        <TableCell className="pl-8">
                          <div className="flex items-center gap-3 py-2">
                             ...
                             <div className="flex flex-col -space-y-0.5">
                                <span className="font-black text-navy-900 text-sm">{item.employeeName}</span>
                        ...
                        <TableCell>
                           <div className="flex flex-col">
                              <span className="font-bold text-navy-900 text-sm">{item.destination}</span>
                              <span className="text-[10px] font-data text-slate-400">{item.scheduleDate}</span>
                           </div>
                        </TableCell>
                        ...
                        <TableCell className="text-right pr-8">
                          <div className="flex items-center justify-end gap-2">
                            <Button variant="ghost" size="icon" onClick={() => setSelectedEntry(item)} className="h-10 w-10 rounded-xl text-slate-400 hover:text-blue-600 hover:bg-blue-50">
            ...
            >
               {filteredData.map((item) => (
                 <Card key={item.id} className="border-none bg-white shadow-xl shadow-navy-900/5 rounded-[2.5rem] overflow-hidden group hover:scale-[1.02] transition-all duration-500 cursor-pointer" onClick={() => setSelectedEntry(item)}>
                    <CardHeader className="p-8 pb-4">
                      <div className="flex justify-between items-start mb-6">
                         <div className="flex flex-col -space-y-1">
                            <span className="micro-label opacity-40">Ref: {item.id?.slice(0, 6)}</span>
                            <span className="font-bold text-navy-900">{item.employeeName}</span>
                         </div>
                    ...
                    <CardContent className="px-8 pb-8 space-y-6">
                      ...
                      <div className="pt-4 border-t border-slate-50 flex items-center justify-between">
                         <div className="space-y-1">
                            <div className="micro-label">Budget Grant</div>
                            <div className="text-xl font-black font-data text-navy-900">₱{item.requestedCashAdvance?.toLocaleString()}</div>
                         </div>
                         {item.liquidationItems && (
                           <div className="text-right space-y-1">
                              ...
                              <div className="text-xl font-black font-data text-emerald-600">
                                 ₱{item.liquidationItems.reduce((s, i) => s + i.amount, 0).toLocaleString()}
                              </div>
                           </div>
                         )}
                      </div>
             ...
             <Table>
                <TableHeader className="bg-slate-50/50">
                  <TableRow className="hover:bg-transparent border-slate-50">
                    ...
                    <TableHead className="font-black text-[9px] uppercase tracking-[0.2em] h-14 text-slate-400">Privilege</TableHead>
                    <TableHead className="font-black text-[9px] uppercase tracking-[0.2em] h-14 text-slate-400">Join Date</TableHead>
                    <TableHead className="font-black text-[9px] uppercase tracking-[0.2em] h-14 text-slate-400 text-right pr-8">Management</TableHead>
                  </TableRow>
                </TableHeader>
                <TableBody>
                   {filteredUsers.length === 0 ? (
                      <TableRow><TableCell colSpan={5} className="text-center py-20 text-xs font-black uppercase text-slate-300 tracking-widest italic">No personnel found.</TableCell></TableRow>
                   ) : (
                     filteredUsers.map((u) => (
                       <TableRow key={u.uid} className="hover:bg-slate-50/80 transition-colors border-slate-50 group">
                          <TableCell className="pl-8">
                             <div className="flex items-center gap-3 py-2">
                                <div className={`w-10 h-10 rounded-xl flex items-center justify-center font-black text-xs ${u.role === 'admin' ? 'bg-red-50 text-red-600 border border-red-100' : 'bg-slate-100 text-navy-900'}`}>
                                   {u.displayName?.charAt(0)}
                                </div>
                                <div className="flex flex-col -space-y-0.5">
                                   <span className="font-black text-navy-900 text-sm">{u.displayName}</span>
                                   <span className="text-[10px] font-bold text-slate-400 lowercase">{u.email}</span>
                                </div>
                             </div>
                          </TableCell>
                          <TableCell>
                             <Badge variant="outline" className="rounded-md border-slate-100 text-slate-500 font-bold text-[9px] uppercase tracking-widest">
                                {u.department}
                             </Badge>
                          </TableCell>
                          <TableCell>
                          ...
                          <TableCell className="text-right pr-8">
                             <DropdownMenu>
                                ...
                                <DropdownMenuContent align="end" className="rounded-2xl p-2 min-w-[200px]">
                                   ...
                                   {u.role === 'user' ? (
                                     <DropdownMenuItem onClick={() => handleUpdateUserRole(u.uid, 'admin')} className="gap-3 text-red-600 cursor-pointer rounded-xl font-bold">
                                        <Lock className="h-4 w-4" /> Promote to Admin
                                     </DropdownMenuItem>
                                   ) : (
                                     <DropdownMenuItem onClick={() => handleUpdateUserRole(u.uid, 'user')} className="gap-3 text-slate-600 cursor-pointer rounded-xl font-bold">
                                        <Unlock className="h-4 w-4" /> Demote to User
                                     </DropdownMenuItem>
                                   )}
                                   <DropdownMenuSeparator className="bg-slate-100" />
                                   <DropdownMenuItem onClick={() => handleDeleteUser(u.uid)} className="gap-3 text-red-500 cursor-pointer rounded-xl font-bold hover:bg-red-50 transition-colors">
